...
```
설명 : allocateDirect() 메소드를 통해 Direct Buffer를 생성하면, 네이티브 메모리에 버퍼가 할당된다. 네이티브 메모리에 직접 접근하기 때문에 입출력 작업에서 성능이 뛰어나다. ```put()```과 ```get()``` 메소드를 통해 데이터를 쓸 때마다 빠르게 읽기와 쓰기가 가능하다.

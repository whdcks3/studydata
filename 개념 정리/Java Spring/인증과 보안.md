...
----------------
### 세션 기반 인증의 장점
보안성: 세션 데이터는 서버에서 관리되므로, 클라이언트가 직접 조작할 수 없음.
사용자 상태 유지: 서버가 사용자의 상태를 지속적으로 유지하므로, 인증 상태를 쉽게 확인 가능.
자동 로그아웃: 일정 시간이 지나면 세션이 만료되도록 설정할 수 있어 보안성을 강화할 수 있음.
세션 기반 인증의 단점
확장성 문제: 세션은 서버에서 관리되므로, 사용자가 많아지면 서버의 메모리를 차지하게 되어 부하가 증가함.
로드 밸런싱 문제: 다수의 서버가 운영되는 환경에서는 세션을 공유해야 하므로 추가적인 설정이 필요함.
...
/auth/login 엔드포인트를 호출하면 사용자의 인증을 처리하고 세션을 생성한다.
인증이 완료되면, 서버는 세션 ID를 발급하고 이를 브라우저 쿠키에 저장한다.
/auth/logout 엔드포인트를 호출하면 세션이 만료되고 로그아웃이 처리된다.
토큰 기반 인증(Token-Based Authentication)
토큰 기반 인증(Token-Based Authentication) 은 서버에서 사용자의 인증 상태를 유지하는 대신,
클라이언트에게 토큰(Token) 을 발급하여 인증하는 방식이다.
...
서버는 사용자의 정보를 데이터베이스에서 조회하여 검증함.
...
이후 사용자가 다른 요청을 보낼 때, HTTP 헤더에 토큰을 포함하여 전송함.
서버는 전달된 토큰을 검증하여 사용자의 신원을 확인함.
토큰 만료 또는 갱신

토큰은 만료 시간이 정해져 있으며, 만료되면 새로운 토큰을 발급해야 함.
...
JWT 인증 흐름 요약
사용자가 로그인 정보를 서버에 제출 → 서버에서 인증 후 JWT 발급
클라이언트는 JWT를 저장하고 API 요청 시 포함
...
Header, Payload, Signature의 역할을 설명해보라.
JWT의 Payload에 저장하는 정보가 많아지면 어떤 문제가 발생할 수 있는가?

토큰 크기 증가에 따른 문제점을 고려해보라.
...
JwtAuthenticationFilter를 UsernamePasswordAuthenticationFilter 앞에 추가한다.
이를 통해 클라이언트 요청이 Spring Security 필터 체인을 거칠 때 JWT 검증이 먼저 실행된다.
CSRF 보호 비활성화 (csrf().disable())

JWT 기반 인증에서는 세션을 사용하지 않기 때문에 CSRF 보호가 불필요하다.
만약 CSRF 보호가 필요하다면 별도로 설정해야 한다.
세션 정책을 Stateless 모드로 설정 (sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS))

JWT 기반 인증에서는 서버가 세션을 저장하지 않는다.
따라서 STATELESS 모드로 설정하여 클라이언트가 매 요청마다 JWT를 포함하도록 한다.
보호된 엔드포인트 설정 (authorizeHttpRequests())

/api/auth/** 엔드포인트는 인증 없이 접근할 수 있도록 permitAll()을 설정.
...
JWT에서 사용자 정보 추출 및 인증 객체 생성

jwtTokenProvider.getUsernameFromToken(token)을 통해 사용자의 아이디를 가져온다.
userDetailsService.loadUserByUsername(username)을 통해 사용자 정보를 로드한다.
...
사용자가 로그인 요청을 보냄 (/api/auth/login)

서버에서 사용자 정보를 검증 후 JWT를 생성하여 응답.
클라이언트는 JWT를 저장하고 API 요청 시 Authorization 헤더에 포함

이후 API 요청마다 JWT를 Bearer Token으로 전송.
Spring Security 필터 체인에서 JwtAuthenticationFilter가 실행

Authorization 헤더에서 JWT를 추출하고 검증.
토큰이 유효한 경우 사용자 정보를 SecurityContext에 저장.
보호된 엔드포인트에 대한 요청을 처리
...
SecurityConfig 클래스를 생성한다.
Spring Security의 필터 체인에 JWT 인증 필터(JwtAuthenticationFilter)를 추가한다.
모든 요청을 인증하도록 설정하되, /login 엔드포인트는 인증 없이 접근 가능하도록 설정한다.
문제 2: JWT 인증 필터 생성
다음 요구사항을 만족하는 JWT 인증 필터 클래스를 구현하시오.

OncePerRequestFilter를 확장하여 JwtAuthenticationFilter 클래스를 생성한다.
HTTP 요청에서 JWT를 추출하고 검증하는 로직을 구현한다.
검증된 사용자의 정보를 SecurityContext에 저장한다.
...
JWT 생성 로직 설명
사용자가 로그인 요청을 하면 authenticate(username, password)를 호출한다.
authenticationManager.authenticate(...)를 사용하여 사용자의 자격 증명을 확인한다.
사용자의 정보를 로드한 후, jwtTokenProvider.generateToken(userDetails)를 호출하여 JWT를 생성한다.
...
JWT 생성 과정
setSubject(userDetails.getUsername()) → JWT의 subject 필드에 사용자명을 저장한다.
setIssuedAt(new Date()) → JWT가 발급된 시간을 저장한다.
setExpiration(...) → JWT의 만료 시간을 설정한다.
signWith(SignatureAlgorithm.HS256, JWT_SECRET) → HMAC SHA-256 알고리즘으로 JWT에 서명한다.
...
JWT 검증 과정 설명
Jwts.parser().setSigningKey(JWT_SECRET).parseClaimsJws(token);
전달된 JWT가 올바르게 서명되었는지 검증한다.
예외 처리:
ExpiredJwtException → 토큰이 만료된 경우 예외 발생
MalformedJwtException → 토큰 형식이 올바르지 않은 경우 예외 발생
SignatureException → JWT의 서명이 올바르지 않은 경우 예외 발생
IllegalArgumentException → 잘못된 값이 전달된 경우 예외 발생
...
JWT에서 사용자 정보 추출 과정
JWT를 파싱하여 setSigningKey(JWT_SECRET)로 서명을 검증한다.
...
로그인 API 동작 과정
클라이언트가 /api/auth/login 엔드포인트로 username과 password를 전송한다.
AuthService.authenticate()를 호출하여 인증을 진행한다.
인증이 성공하면 jwtTokenProvider.generateToken(userDetails)를 호출하여 JWT를 생성한다.
생성된 JWT를 AuthResponse 객체에 담아 클라이언트에 응답한다.
5.3. 인증 필터 적용 및 Spring Security 연동
Spring Security에서 JWT 기반 인증을 사용하려면 JWT 인증 필터를 구현하여 HTTP 요청마다 JWT를 검증해야 한다. 이를 위해 OncePerRequestFilter를 활용하여 JWT를 확인하고, 검증된 사용자 정보를 SecurityContext에 저장하는 작업이 필요하다.
...
Spring Security에서는 UsernamePasswordAuthenticationToken을 주로 사용하지만, JWT 인증에서는 비밀번호 검증이 필요하지 않으므로 새로운 JwtAuthenticationToken 클래스를 만들어 사용한다.
...
JwtAuthenticationToken 역할
Spring Security에서 인증 객체를 다루기 위한 클래스
getCredentials() 메서드는 JWT 기반 인증에서는 필요하지 않으므로 null 반환
...
세션을 사용하지 않도록 설정 (sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS))
JWT는 Stateless 방식이므로, 세션을 사용하지 않도록 설정.
API 엔드포인트별 인증 설정
/api/auth/login은 인증 없이 접근 가능하도록 permitAll()
나머지 API 요청은 authenticated()로 보호.
JWT 인증 필터 추가
addFilterBefore(jwtAuthenticationFilter, UsernamePasswordAuthenticationFilter.class)
기존 Spring Security의 UsernamePasswordAuthenticationFilter보다 먼저 실행되도록 필터 등록.
JWT 인증 적용 후 API 요청 흐름
사용자가 /api/auth/login 엔드포인트로 username과 password를 전송.
AuthService에서 사용자 정보를 검증하고 JWT를 생성하여 클라이언트에 반환.
이후 API 요청 시, 클라이언트는 JWT를 "Authorization" 헤더에 포함하여 요청을 보냄.
Authorization: Bearer eyJhbGciOiJIUzI1NiIsIn...
JwtAuthenticationFilter가 요청을 가로채서:
JWT가 유효한지 검증
JWT에서 사용자 정보 추출
...
generateToken(String username) 메서드를 작성하여 사용자명을 포함하는 JWT를 생성한다.
토큰의 만료 시간을 1시간으로 설정한다.
문제 2: JWT 검증 서비스 구현
다음 요구사항을 만족하는 JWT 검증 클래스를 구현하시오.

validateToken(String token) 메서드를 작성하여 JWT의 유효성을 검증한다.
토큰이 만료되었거나 위조되었을 경우 예외를 발생시키도록 처리한다.
6. API 요청 보호 및 권한 관리
6.1. 특정 API 보호 설정
...
설정 설명
@PreAuthorize("hasRole('USER')"):
/api/user/profile API는 USER 역할이 있는 사용자만 접근 가능.
@PreAuthorize("hasRole('ADMIN')"):
/api/admin/data API는 ADMIN 역할이 있는 사용자만 접근 가능.
@PostAuthorize("returnObject.owner == authentication.name"):
반환된 객체의 owner 속성이 현재 로그인한 사용자와 일치하는 경우에만 접근 허용.
스프링 시큐리티에서 @PreAuthorize를 활성화하는 방법
Spring Security에서 @PreAuthorize와 @PostAuthorize를 사용하려면 @EnableMethodSecurity를 활성화해야 한다.
...
@EnableMethodSecurity를 추가하면 @PreAuthorize와 @PostAuthorize를 사용할 수 있다.

보호된 API 요청 흐름
클라이언트가 /api/user/profile을 요청.
요청 헤더에 JWT 포함.
Authorization: Bearer eyJhbGciOiJIUzI1NiIsIn...
JwtAuthenticationFilter에서 JWT 검증 후 SecurityContext에 사용자 정보 저장.
Spring Security에서 요청된 API의 접근 권한 확인:
@PreAuthorize("hasRole('USER')") 설정이 적용된 경우, USER 역할이 있는지 확인.
@PreAuthorize("hasRole('ADMIN')") 설정이 적용된 경우, ADMIN 역할이 있는지 확인.
...
403 Forbidden:
사용자가 요청을 보냈지만 권한이 없어서 접근이 거부됨.
JWT가 없거나, ROLE_USER 권한이 없는 경우 발생.
API 보호 설정의 핵심 포인트
HTTP 요청 수준 보호 (authorizeHttpRequests())
특정 엔드포인트는 permitAll(), 특정 역할 필요 시 hasRole('USER') 사용.
메서드 수준 보호 (@PreAuthorize 및 @PostAuthorize)
세밀한 권한 관리를 위해 컨트롤러의 메서드에 직접 적용.
Spring Security 필터 체인과 함께 JWT 인증 적용
JwtAuthenticationFilter에서 JWT 검증 후 SecurityContext에 인증 정보 저장.
학습자의 사고를 돕기 위한 질문
API의 보안을 강화하기 위해 특정 엔드포인트에 접근을 제한하는 방법은 무엇인가?

Spring Security에서 제공하는 접근 제어 방식을 고려해보라.
관리자 전용 엔드포인트(/admin)와 사용자 전용 엔드포인트(/user)를 나눠서 설정하는 방법은 무엇인가?

역할 기반 접근 제어(Role-based Access Control)의 개념을 생각해보라.
실습 문제
문제 1: 특정 API 보호 설정
다음 요구사항을 만족하는 Spring Security 설정을 구현하시오.
...
6.2. Role 기반 접근 제어
Spring Security에서는 사용자의 역할(Role)에 따라 API 접근을 제한하는 기능을 제공한다. 이 기능을 통해 각 사용자의 권한 수준을 설정하고, 특정 역할(Role)이 있는 사용자만 특정 API에 접근하도록 설정할 수 있다. 이를 설정하는 방식에는 다음과 같은 방법이 있다.
...
Spring Security에서는 특정 URL 패턴에 따라 접근을 제한할 수 있다. 이를 위해 SecurityConfig에서 authorizeHttpRequests() 메서드를 활용한다.
...
Spring Security에서는 특정 API의 메서드 실행 전에 사용자의 Role을 검사하여 접근을 제한할 수도 있다. 이를 위해 @PreAuthorize와 @PostAuthorize를 사용한다.

@PreAuthorize: 메서드 실행 전에 Role을 검사.
